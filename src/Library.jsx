...
const LEGACY_LIBRARY_TAG_BLOCKLIST = new Set(
...
const extractCustomSoundTags = (tags) => {
  ...
  for (const raw of tags) {
    ...
    if (!tag) continue;
    const lower = tag.toLowerCase();
    if (LEGACY_SOUND_POSITION_SET.has(lower) || isLegacyLibraryTag(tag)) {
      continue;
    }
    const triPreset = resolveTriPresetTag(tag);
    if (
...
const isLegacyLibraryTag = (value) => {
  const tag = sanitizeTag(value);
  if (!tag) return false;
  return LEGACY_LIBRARY_TAG_BLOCKLIST.has(tag.toLowerCase());
};
...
const canonicalizeTags = (tags) => {
  ...
  tags.forEach((raw) => {
    const tag = sanitizeTag(raw);
    if (!tag) return;
    if (isLegacyLibraryTag(tag)) return;
    const lower = tag.toLowerCase();
    if (!seen.has(lower)) {
      output.push(tag);
      seen.add(lower);
